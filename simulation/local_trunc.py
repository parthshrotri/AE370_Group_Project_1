...
def two_body_local_trunc(delta_t, days, baseline_last):
    ...
    appx_traj = np.load(os.path.join(os.path.dirname(__file__), f'../output/two_body_test_{delta_t}/', 'earth_trajectory.npy'))
...
def two_body_baseline(baseline_delta_t, days = 365):
    ...
    baseline_traj = np.load(os.path.join(os.path.dirname(__file__), f'../output/two_body_test_{baseline_delta_t}/', 'earth_trajectory.npy'))
...
def three_body_local_trunc(delta_t, days, baseline_last):
    ...
    earth_appx_traj = np.load(os.path.join(os.path.dirname(__file__), f'../output/three_body_test_{delta_t}/', 'earth_trajectory.npy'))
    earth_appx_last = earth_appx_traj[-1]
    moon_appx_traj = np.load(os.path.join(os.path.dirname(__file__), f'../output/three_body_test_{delta_t}/', 'moon_trajectory.npy'))
...
def three_body_baseline(baseline_delta_t, days = 365):
    ...
    earth_baseline_traj = np.load(os.path.join(os.path.dirname(__file__), f'../output/three_body_test_{baseline_delta_t}/', 'earth_trajectory.npy'))
    earth_baseline_last = earth_baseline_traj[-1]
    moon_baseline_traj = np.load(os.path.join(os.path.dirname(__file__), f'../output/three_body_test_{baseline_delta_t}/', 'moon_trajectory.npy'))
...
with mp.Pool() as pool:
    pool.map(multiprocess, delta_t_vals)
